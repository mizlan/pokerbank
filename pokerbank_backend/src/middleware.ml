...
let auth_player_middleware next_handler req =
  ...
  | None -> R.unauthorized "not signed in"
  | Some player_id -> (
      let player_id = Int.of_string player_id in
      match player_id with
      | None -> R.unauthorized "malformed player_id"
      | Some player_id ->
...
let auth_bank_middleware next_handler req =
  ...
  | None -> R.unauthorized "no player_id"
  | Some player_id -> (
      let* session_id = Dream.sql req (Db.get_session_of_player ~player_id) in
      match session_id with
      | Error msg -> R.internal msg
      | Ok session_id -> (
          match session_id with
          | None -> R.forbidden "not a bank player"
          | Some session_id ->
